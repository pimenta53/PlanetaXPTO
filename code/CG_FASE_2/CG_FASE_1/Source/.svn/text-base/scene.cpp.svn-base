...
void drawBala(int i){
	...
	balas[i][2] += vel*sin(degToRad(-balas[i][0])); // posição z
	balas[i][1] += vel*cos(degToRad(-balas[i][0])); // posição x
...
/*Função que desenha o tesouro,no vale, no extremo oposto ao agente*/
void drawCastelo() {
