...
Md2Player::Md2Player (const string &dirname, const string &obj)
  ...
  while ((dit = readdir (dd)) != NULL)
	  ...
	  !((str[l-1] == 'i') && (str[l-2] == '_')))
	{
	  ...
	  if (filename.compare (l, 4, ".pcx") == 0 ||
	      ...
	      filename.compare (l, 4, ".TGA") == 0)
	    {
	      if (filename.compare (0, modelLen, obj) == 0)
		{
			cout << path + "\n";
